...
def get_events(df: pd.DataFrame):
    workload_df = df[
        df["process name"].isin(
            ["ferret", "dedup", "canneal", "freqmine", "radix", "blackscholes", "vips"]
        )
    ]
    return workload_df
...
def get_time_for_job(df, name):
    start_time = df[(df["process name"] == name) & (df["event"] == "START")][
        "timestamp"
    ]
...
def main():
    ...
    for run in range(3):
        ...
        timestamps = list(
            range(10, controller_time_end - controller_time_start + 20, 20)
        )
        ...
        event_labels = [
            row["process name"] + ": " + row["event"] for _, row in events.iterrows()
        ]
        ...
        for idx, name in enumerate(workloads):
            ...
            for i in range(0, len(entries), 2):
                if len(entries) % 2 == 1 and i == len(entries) - 3:
                    ax_events.plot(
                        [
                            entries.iloc[i]["timestamp"],
                            entries.iloc[i + 2]["timestamp"],
                        ],
                        [idx, idx],
                        color=color,
                        linewidth=3,
            ...
            ax_events.scatter(
                [entries.iloc[0]["timestamp"]], [idx], c=color, marker="o"
            )
            ax_events.scatter(
                [entries.iloc[-1]["timestamp"]], [idx], c=color, marker="x"
            )
        ...
        (artistA_95p,) = axA_95p.plot(
            df_lat["timestamp"], df_lat["p95"], "o-", color="tab:blue"
        )
        axA_QPS = axA_95p.twinx()
        artistA_QPS = config_QPS_ax(axA_QPS)
        axA_QPS.legend(
            [artistA_QPS, artistA_95p],
            ["QPS", "95 percentile latency"],
            loc="upper right",
        )
